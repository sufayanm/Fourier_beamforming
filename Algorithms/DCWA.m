...
% Implements the Delay-and-sum consistent wavenumber algorithm for ultrasound imaging.
%   
% Inputs:
...
mult_limit = 0.007 ;   % Limit maximum and minimum values in the multiplication factor
% Very large values in mult_factor matrix can distort the final image.
% mult_limit variable puts an upper and lower bound (-mult_limit) on the mult_factor matrix 
% This is an arbitrary value chosen after considering the ditribution of mult_factor matrix. 

mult_factor(isinf(mult_factor)) = mult_limit*sign(mult_factor(isinf(mult_factor))) ;
mult_factor(isnan(mult_factor)) = 0 ;
mult_factor(mult_factor<-mult_limit) = -mult_limit ;
mult_factor(mult_factor>mult_limit) = mult_limit;
...
%% Scale the reconstructed image with z coordinate
